...
bool CFbTcComExceptionGenerator::FB_init(bool bInitRetains, bool bInCopyCode, ST_FB_ExceptionGenerator_TcCOM_InitStruct stInit)
{
	...
	if(SUCCEEDED(hr))
	{
		   ...
		   stInit.nOid == OTCID_LastFreeId)
		{
			...
			if (SUCCEEDED(hr))
			{
				hr = m_ipObj->TcSetObjectName(m_InstancePath);
				if (FAILED(hr))
...
{
	((CFbTcComExceptionGenerator*)ST_Param->pInst)->FB_init( ST_Param->bInitRetains, ST_Param->bInCopyCode, ST_Param->stInit );
}
...
{
	((CFbTcComExceptionGenerator*)ST_Param->pInst)->FB_afterinit(  );
}
...
{
	((CFbTcComExceptionGenerator*)ST_Param->pInst)->Reinit( ST_Param->stReInit );
}
...
{
	((CFbTcComExceptionGenerator*)ST_Param->pInst)->FB_Main(  );
}
...
{
	((CFbTcComExceptionGenerator*)ST_Param->pInst)->FB_exit( ST_Param->bInCopyCode );
}
...
HRESULT CFbTcComExceptionGenerator::GetnCycleCount(ULONGLONG& nParam)
{
	...
	if SUCCEEDED(hr)
	{
		...
		if FAILED(hr)
			Log(TcTraceLevel::tlError, "TcGetObjPara of nCycleCount (ULONGLONG) with PID 0x0000000C failed: hr = 0x%08X",hr);
	}
	return hr;
...
HRESULT CFbTcComExceptionGenerator::GetnExceptionCount(ULONGLONG& nParam)
{
	...
	if SUCCEEDED(hr)
	{
		...
		if FAILED(hr)
			Log(TcTraceLevel::tlError, "TcGetObjPara of nExceptionCount (ULONGLONG) with PID 0x0000000D failed: hr = 0x%08X",hr);
	}
	return hr;
...
HRESULT CFbTcComExceptionGenerator::GetstExeceptionInfo(TcMgSdk::ExceptionInfo& stParam)
{
	HRESULT hr =  m_ipObj ? S_OK : ADS_E_NOINTERFACE;
	PVOID pparam = &stParam;
	ULONG nparam = sizeof(stParam);
	if SUCCEEDED(hr)
	{
		...
		if FAILED(hr)
			Log(TcTraceLevel::tlError, "TcGetObjPara of stExeceptionInfo (TcMgSdk::ExceptionInfo) with PID 0x0000000E failed: hr = 0x%08X",hr);
	}
	return hr;

}

void CFbTcComExceptionGenerator::_GetstExeceptionInfo(ST_GetSet_ST_ExceptionInfo* ST_Param)
{
	((CFbTcComExceptionGenerator*)ST_Param->pInst)->GetstExeceptionInfo( ST_Param->stParam );
}
