...
class CExceptionGeneratorCtrl  : public ITcOCFCtrlImpl< CExceptionGeneratorCtrl, CExceptionGeneratorClassFactory>
{
	public:
	CExceptionGeneratorCtrl () {};
...
extern "C" __declspec(dllexport) HRESULT APIENTRY DllGetTcCtrl(ITcCtrl** ppTcCtrl)
{
	...
	if (ppTcCtrl != NULL)
	{
		ITcCtrl* pTcCtrl = new IUnknownImpl<CExceptionGeneratorCtrl>;
		pTcCtrl->AddRef();
		*ppTcCtrl = pTcCtrl;
		hr = S_OK;
	}
